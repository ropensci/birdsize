test_that("pop_generate errors work", {
  expect_error(pop_generate(species_mean = 100, species_sd = 10), regexp = "`species_abundance` must be provided")

  expect_error(pop_generate(species_abundance = "10", species_mean = 100, species_sd = 10), regexp = "`species_abundance` must be numeric")

  expect_error(pop_generate(species_abundance = 10.5, species_mean = 100, species_sd = 10), regexp = "`species_abundance` must be a whole number")

  expect_error(pop_generate(species_abundance = 10), regexp = "Either `species_mean` or `species_code` must be provided.")

  expect_error(pop_generate(species_abundance = 10, species_code = "abc"), regexp = "`species_code` is invalid.")

  expect_error(pop_generate(species_abundance = 10, species_code = 1000), regexp = "`species_code` is invalid.")

  expect_message(pop_generate(species_abundance = 10, species_mean = 10, species_sd = 2, species_code = 1420), regexp = "Both `species_code` and `species_mean` are provided; using `species_code` and overwriting `species_mean` based on species' parameters.")
})
...
test_that("pop_generate works given mean and sd", {
  ...
  a_population <- pop_generate(species_abundance = 100, species_mean = 10, species_sd = 2)
...
test_that("pop_generate works given only mean", {
  ...
  a_population <- pop_generate(species_abundance = 100, species_mean = 10)
...
test_that("pop_generate works given species code", {
  ...
  a_population <- pop_generate(species_abundance = 100, species_code = 1420)
...
test_that("pop_generate works given species code *and* species mean", {
  ...
  a_population <- pop_generate(species_abundance = 100, species_code = 1420, species_mean = 20)

...
species_define <- function(aou = NULL, genus = NULL, species = NULL, mean_size = NULL, sd_size = NULL, id = 1) {
  ...
  if(!is.null(aou)) {
    ...
    if(!is.null(genus)) {
      ...
      if(genus != thisSpecies$genus) { warning("User-provided genus does not match genus associated with this AOU") }

    }

    if(!is.null(species)) {

      if(species != thisSpecies$species) { warning("User-provided species does not match species associated with this AOU") }

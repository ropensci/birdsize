...
The core functionality of `birdsize` is to simulate individual body mass measurements for birds given either their species ID or a mean (and possibly standard deviation) of body mass. `pop_generate` achieves this for a single species.
...
To look up the AOU based on the species' scientific name, explore the [list](https://www.pwrc.usgs.gov/bbs/specieslist.html) maintained by the USGS, or the included `sd_table` dataset:
